...
impl ServerRenderer {
    ...
    fn render_uncached_with_env(
        ...
        let html = if route.route.mode.cache_scope().is_publicly_shareable() {
            let route_stylesheet_href = self.remember_route_css(&css)?;
            replace_stylesheet_href(
                rendered.html,
                &global_stylesheet_href,
                &route_stylesheet_href,
            )?
        } else {
            replace_stylesheet_with_inline_css(rendered.html, &global_stylesheet_href, &css)?
        };
...
fn replace_stylesheet_with_inline_css(
    html: String,
    current_href: &str,
    css: &str,
) -> Result<String> {
    ...
    Ok(html.replacen(
        &current,
        &format!("<style data-fission-route-styles>{css}</style>"),
        1,
    ))
}
...
mod tests {
    ...
    fn private_server_routes_do_not_write_full_page_public_cache_entries() {
        ...
        assert!(first
            .body_string()
            .contains("<style data-fission-route-styles>"));
        assert!(!first.body_string().contains(ROUTE_STYLESHEET_PREFIX));
    }
